...
```hcl
module "lambda-api" {
    source = "github.com/pbs/terraform-lambda-api-module?ref=x.y.z"
}
```
...
```bash
mkdir -p 'terraform/modules/lambda-api'
gh -R 'pbs/terraform-lambda-api-module' release download -p 'release.tar.gz' x.y.z
tar -xvf release.tar.gz -C 'terraform/modules/lambda-api'
rm -f release.tar.gz
```
...
```bash
git remote add -f lambda-api git@github.com:pbs/terraform-lambda-api-module.git
git subtree add --prefix terraform/modules/lambda-api lambda-api main --squash
```

If necessary, pin the repo version, rather than adding `main` as your subtree:

```bash
git subtree add --prefix terraform/modules/lambda-api lambda-api $REF --squash
```
